...
import { ArrowRight, MapPin, Sparkles, KeyRound, Utensils } from "lucide-react";
      ...
      <div className="mx-auto max-w-7xl px-6">
        <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-12 lg:gap-8">
          ...
          <div className="lg:col-span-5 space-y-6 z-10">
            ...
            <div className="inline-flex flex-wrap items-center gap-2 rounded-pill bg-bg-subtle border border-divider px-3.5 py-1.5 text-xs text-brand-primary shadow-2xs">
              <span className="flex items-center gap-1 text-brand-accent font-semibold">
                <Sparkles className="h-3.5 w-3.5" />
                <span>Now Open Daily</span>
              </span>
            ...
            <div className="space-y-2">
              <span className="text-xs font-semibold tracking-[0.25em] text-brand-accent uppercase block">
                Tibeb Ethiopian Gastronomy
              </span>
              <h1 className="font-header text-4xl sm:text-5xl lg:text-6xl font-bold tracking-tight text-brand-heading leading-[1.08]">
                {RESTAURANT_INFO.name}
              </h1>
              <p className="font-header text-lg sm:text-xl font-medium text-brand-secondary">
                {RESTAURANT_INFO.amharicName} • Fine Hospitality
              </p>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              <Link
                href="/menu"
                className="min-h-[48px] inline-flex items-center justify-center gap-2 rounded-button bg-brand-accent px-6 py-3.5 text-sm font-bold text-white shadow-md transition-all duration-150 ease-out hover:bg-brand-accentHover hover:-translate-y-0.5 active:scale-95"
              >
              ...
              <Link
                href="/rms-login"
                className="min-h-[48px] inline-flex items-center justify-center gap-2 rounded-button bg-bg-card px-5 py-3.5 text-sm font-semibold text-brand-primary border border-divider transition-all duration-150 ease-out hover:bg-bg-active hover:-translate-y-0.5"
              >
            ...
            <div className="grid grid-cols-3 gap-4 border-t border-divider pt-6 text-xs text-brand-secondary">
              <div>
                <span className="font-header text-xl font-bold text-brand-primary block">
                  100%
                </span>
          ...
          <div className="lg:col-span-7 relative">
            <div className="relative mx-auto w-full rounded-card overflow-hidden shadow-elevated border border-divider">
              ...
              <div className="relative h-[380px] sm:h-[460px] lg:h-[540px] w-full bg-bg-subtle">
                <img
                  src="https://images.unsplash.com/photo-1544025162-d76694265947?auto=format&fit=crop&w=1400&q=85"
                  alt="Tibeb Ethiopian wood-fired hearth dining"
